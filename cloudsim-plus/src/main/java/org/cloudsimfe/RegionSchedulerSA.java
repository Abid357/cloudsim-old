...
public class RegionSchedulerSA implements RegionScheduler {
    ...
    public RegionSchedulerSA(UnifiedManager manager) {
    ...
    public boolean scheduleRegions(int fpgaCount, int regionCount, Object tasks) {
        ...
        this.jobs = (List<Job>) tasks;
        NUM_OF_TASKS = jobs.size();
    ...
    private void simulate() {
        ...
        int currentEnergy = -1;
        int[] currentSolution = allocate(jobs, true);
        bestSolution = currentSolution;
        int bestEnergy = getCompletionTime(bestSolution);
        initialSolution = currentSolution;

        List<Job> newJobs = new ArrayList<Job>(jobs);
        List<Job> prevJobs = new ArrayList<Job>(newJobs);
        ...
        while (temp > 1 && ITERATION_COUNT != ITERATION_THRESHOLD) {
            ...
            for (int i = 0; i < newJobs.size() / 3; i++) {
                // get a random positions
                int pos1 = (int) (newJobs.size() * Math.random());
                int pos2 = (int) (newJobs.size() * Math.random());

                // swap jobs based on positions
                Job tempJob = newJobs.get(pos1);
                newJobs.set(pos1, newJobs.get(pos2));
                newJobs.set(pos2, tempJob);

                // get energy of solutions
                currentEnergy = getCompletionTime(currentSolution);
                int[] newSolution = allocate(newJobs, false);
                ...
                if (neighbourEnergy == -1) {
                    newJobs = new ArrayList<Job>(prevJobs);
                    continue;
                }
                prevJobs = new ArrayList<Job>(newJobs);
...
//                break;
    ...
    private int getTMax() {
        if (!jobs.isEmpty())
            return jobs.stream().mapToInt(Job::getExecutionTime).sum();
        else
            return -1;
    }
    ...
    private int[] allocate(List<Job> jobs, boolean initialFlag) {
        int[] x = new int[T_MAX * NUM_OF_REGIONS];
        for (int k = 0; k < jobs.size(); k++) {
            int id = jobs.get(k).getId();
            int requiredTime = jobs.get(k).getExecutionTime();
            int requiredTiles = jobs.get(k).getTile();
            int currentTime = 1;
            int pos = -1;
            ...
            for (; !success && currentTime <= T_MAX; currentTime++) {
                if (!initialFlag && (currentTime + jobs.get(k).getExecutionTime() - 1) > jobs.get(k).getDeadline())
                    return null;
