 ...
 * Datacenter, each SegmentExecutionInfo object represents the data about
...
public class SegmentExecution {
    /**
     * A property that implements the Null Object Design Pattern for {@link SegmentExecution}
     * objects.
     */
    // public static final SegmentExecution NULL = new SegmentExecution(Cloudlet.NULL);
    ...
    private AccelerableSegment segment;

    /**
     * Instantiates a SegmentExecutionInfo object upon the arrival of a Cloudlet inside a Datacenter.
     * The arriving time is determined by {@link CloudSim#clock()}.
     *
     * @param segment the AccelerableSegment to store execution information from
     */
    public SegmentExecution(final AccelerableSegment segment) {
        this.segment = segment;
        if (segment != null) {
            this.cloudlet = segment.getCloudlet();
        ...
        this.instructionsFinishedSoFar = 0;
    }
     ...
     */
    public long getRemainingSegmentLength() {
        final long absLength = Math.abs(segment.getLength());
     ...
     *                                    added to the {@link #instructionsFinishedSoFar}, in <b>Number of
     *                                    Instructions (instead of Million Instructions)</b>
     */
     ...
     * @return arrival time
     */
    public double getSegmentArrivalTime() {
        return arrivalTime;
    }
    ...
    public AccelerableSegment getSegment() {
        return segment;
    }
    ...
    public String toString() {
        return String.format("Segment %d in Cloudlet %d", segment.getId(), cloudlet.getId());
    }

    @Override
    public boolean equals(Object obj) {
        return obj instanceof SegmentExecution &&
                ((SegmentExecution) obj).segment.getId() == this.segment.getId();
    }

    @Override
    public int hashCode() {
        return Objects.hash(segment.getId());
    }
